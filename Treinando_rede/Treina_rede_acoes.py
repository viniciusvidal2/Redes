...
InputNumber = 6
Dias_previstos = 3
Out_posi = np.array([0, 1, 2, 3, 4, 5]) # variaveis a serem previstas - olhar dentro da funcao

# Organizando dados
print('Organizando dados...')
Data = fl.ReadData(filePath)
Data_norm = fl.Normalize(Data)
X, Y = fl.OrganizeData(Data_norm, SampleSize, InputNumber, 1, Out_posi)

X_complexa = X.reshape(X.shape[0], SampleSize, InputNumber) # Para ser iterativo na rede complexa
...
dias_teste = 11 # um a mais que o tanto de dias que queremos
(testx, testy) = X[-dias_teste:-1], Y[-dias_teste:-1]
testx_complexa = X_complexa[-dias_teste:-1]
...
treinar = False
if treinar:
    epocas = 1500 # por quantas epocas treinar
    ...
    rede = Rede_complexa.montar(Subsequences, SampleSize, InputNumber, len(Out_posi))
...
plt.figure()
plt.plot(np.arange(0, saida_real.shape[0] ), saida_real[:, 0] , label="Abertura Real")
plt.plot(np.arange(0, saida_teste.shape[0]), saida_teste[:, 0], label="Abertura Calculado")
plt.title('Abertura')
plt.xlabel('Dia')
...
plt.figure()
plt.plot(np.arange(0, saida_real.shape[0] ), saida_real[:, 1] , label="Fechamento Real")
plt.plot(np.arange(0, saida_teste.shape[0]), saida_teste[:, 1], label="Fechamento Calculado")
...
plt.figure()
plt.plot(np.arange(0, saida_real.shape[0] ), saida_real[:, 2] , label="Maximo Real")
plt.plot(np.arange(0, saida_teste.shape[0]), saida_teste[:, 2], label="Maximo Calculado")
plt.title('Maximo')
plt.xlabel('Dia')
plt.ylabel('Valor_norm')
plt.legend()
...
plt.figure()
plt.plot(np.arange(0, saida_real.shape[0] ), saida_real[:, 3] , label="Minimo Real")
plt.plot(np.arange(0, saida_teste.shape[0]), saida_teste[:, 3], label="Minimo Calculado")
plt.title('Minimo')
plt.xlabel('Dia')
plt.ylabel('Valor_norm')
plt.legend()
...
plt.figure()
plt.plot(np.arange(0, saida_real.shape[0] ), saida_real[:, 4] , label="Media Real")
plt.plot(np.arange(0, saida_teste.shape[0]), saida_teste[:, 4], label="Media Calculada")
plt.title('Media')
plt.xlabel('Dia')
plt.ylabel('Valor_norm')
plt.legend()
...
plt.figure()
plt.plot(np.arange(0, saida_real.shape[0] ), saida_real[:, 5] , label="Volume Real")
plt.plot(np.arange(0, saida_teste.shape[0]), saida_teste[:, 5], label="Volume Calculado")
plt.title('Volume')
plt.xlabel('Dia')
plt.ylabel('Valor_norm')
plt.legend()
